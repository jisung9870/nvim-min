...
Jenkinsfile에는 Groovy LSP를 붙이지 않는다 (JDK가 필요하다). 대신 Groovy Treesitter
위에 Jenkins Declarative Pipeline 전용 강조를 더하고, **Jenkins 서버가 직접 검증**한다 —
아래 참고.

### Jenkinsfile 검증

`pipeline`, `stages`, `steps`, `agent` 같은 구조 directive, `stage` 선언,
`when`과 `post` 조건은 서로 다른 Treesitter 색으로 표시된다. `sh`, `echo`,
`withCredentials` 같은 실행 step은 일반 Groovy 함수 호출 색을 유지한다.

저장하면 `Jenkinsfile`, `Jenkinsfile.*`, `*.Jenkinsfile`, `*.jenkinsfile`을 Jenkins의
